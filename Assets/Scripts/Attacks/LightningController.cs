...
public class LightningController : AttackBaseController 
{
	protected AttackTimeBar timeBar;
	public GameObject[] lightningFaggots;
	private Vector3[] lightningFaggotsInitForward;
	private Projector[] decalProjectors;
	private float timeCounter = 0;
  public Transform damageRadiusTransform;
	...
	public override void OnInit (Vector3 target, Vector3 targetNormal, int level = 0) 
	{
		...
		timeBar = Instantiate(Resources.Load<AttackTimeBar>("AttackTimeBar"), Vector3.zero, Quaternion.identity);
		timeBar.Init (attackDuration, this.transform.position +  Vector3.up * 3);
	}
	...
	private void UpdateTimeBar()
	{
		...
		timeBar.timePoints = timeCounter;
	}
	...
	IEnumerator FadeAwayLightsAndSounds()
	{
		...
		float fadeTime = 0.4f;
    Vector3 tempRadiusScale = damageRadiusTransform.localScale;
    ...
    while (currentTime < fadeTime)
		{
			...
			timeBar.SetAlpha (factor);
      damageRadiusTransform.localScale = tempRadiusScale * factor;
			currentTime += Time.deltaTime;
			yield return null;
		}
		Destroy (timeBar.gameObject);
	}
}
