...
public class TorchAlwaysBurnController : MonoBehaviour 
{
	...
	void Awake () 
	{
		currentAnimTime = Random.Range(0f, 1f);
		initIntensity = torchLight.intensity;
		isAnimatingForward = Random.Range(0, 2) == 0 ? false : true;

		torchIntensityCutoff = Random.Range(0.45f, 0.55f);
		animSpeed += Random.Range(-0.02f, 0.02f);
