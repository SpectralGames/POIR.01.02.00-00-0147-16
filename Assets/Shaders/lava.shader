Shader "Custom/Lava" {
	...
	SubShader {
		...
		half _Glossiness, _DistortStrength, _WaterSpeed, _WaveHeight, _DistortScale, _BounceHeight, _EmissionValue;
		...
		void surf (Input IN, inout SurfaceOutputStandard o) 
		{
			...
			o.Albedo = lerp(abs(sin(IN.worldPos.x + IN.worldPos.z + _Time[3]*_WaterSpeed) + sin(offset.x*3)*0.8)*1.2,   c.rgb + _Color.rgb, fade-0.1)   *   lerp(refractionColor.rgb, float3(1,1,1), _Alpha);
			...
			o.Emission = tex2D(_MainTex, float2(IN.uv_MainTex.x, y)) * _EmissionValue;
		}
		ENDCG
	}
