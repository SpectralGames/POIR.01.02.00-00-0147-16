...
//		}
		...
		inline fixed4 LightingWrapStandardSpecular(SurfaceOutputStandardSpecular s, half3 viewDir, UnityGI gi)
		{
			 ...
			 // Final add
			 pbr.rgb = pbr.rgb + I * gi.light.color * I * s.Albedo * s.Alpha;
			 //s.Alpha = 0;
			 return pbr;
		}
		...
		void vert(inout appdata_full v, out Input o)
		{
            ...
            //fixed perlinTextureFactor = 1.1 * (tex2Dlod(_WindTex, float4(worldPos.x*0.03 + _Time[0]*0.4, worldPos.z*0.03, 0, 0) ).r - 0.5);
            //half maskFactor = v.texcoord.y + 0.5; //v.vertex.y + 0.5; //tex2Dlod(_WindInfluenceTex, float4(v.texcoord.xy, 0, 0)).r;
			
			float3 worldViewDir = normalize(UnityWorldSpaceViewDir(worldPos));
            v.normal.xyz = lerp(float3(0, 1, 0), v.normal.xyz, 0.3);//mul(unity_WorldToObject, lerp(mul(-worldViewDir, float3(0, -1, 0)), mul(unity_ObjectToWorld, v.normal.xyz), 0.25));
